...
auto array_of_config_key_values = array_of<User_Config_TypeDef>(
	...
	.mouse_movement = CFG(MOTION_MOD_KEY_PRESS_RELEASE, {.x_divisor = SMALL_MOD_DIVISOR, .y_divisor = SMALL_MOD_DIVISOR, .z_divisor = 1,
			.x_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RSHIFT, .keys = {KEY_LEFT,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RSHIFT, .keys = {KEY_RIGHT,0,0} })},
			.y_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RSHIFT, .keys = {KEY_UP,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RSHIFT, .keys = {KEY_DOWN,0,0} })},
			.z_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RALT, .keys = {KEY_UP,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RALT, .keys = {KEY_DOWN,0,0} })}, }),
	.short_release = CFG(NO_REPORT,{}),
	...
	.mouse_movement = CFG(MOTION_MOD_KEY_PRESS_RELEASE, {.x_divisor = BIG_MOD_DIVISOR, .y_divisor = BIG_MOD_DIVISOR, .z_divisor = 1,
			...
			KBRD_PRS_RLS({.modifier_keys = 0, .keys = {KEY_ENTER,0,0} })},
			.y_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RMETA, .keys = {KEY_V,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RCTRL, .keys = {KEY_V,0,0} })},
			.z_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = 0, .keys = {KEY_UP,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = 0, .keys = {KEY_DOWN,0,0} })}, }),
	.short_release = CFG(NO_REPORT,{}),
	.long_release = CFG(NO_REPORT,{}),
	}
	...
	.mouse_movement = CFG(MOTION_MOD_KEY_PRESS_RELEASE, {.x_divisor = BIG_MOD_DIVISOR, .y_divisor = BIG_MOD_DIVISOR, .z_divisor = 1,
			.x_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RCTRL | KEY_MOD_RSHIFT, .keys = {KEY_LEFT,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RCTRL | KEY_MOD_RSHIFT, .keys = {KEY_RIGHT,0,0} })},
			.y_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RCTRL | KEY_MOD_RSHIFT, .keys = {KEY_UP,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RCTRL | KEY_MOD_RSHIFT, .keys = {KEY_DOWN,0,0} })},
			.z_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RALT, .keys = {KEY_UP,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RALT, .keys = {KEY_DOWN,0,0} })}, }),
	.short_release = CFG(NO_REPORT,{}),
	...
	.mouse_movement = CFG(MOTION_MOD_KEY_PRESS_RELEASE, {.x_divisor = BIG_MOD_DIVISOR, .y_divisor = BIG_MOD_DIVISOR, .z_divisor = 1,
			.x_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RALT, .keys = {KEY_TAB,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RALT | KEY_MOD_RSHIFT, .keys = {KEY_TAB,0,0} })},
			.y_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RCTRL, .keys = {KEY_TAB,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RCTRL | KEY_MOD_RSHIFT, .keys = {KEY_TAB,0,0} })},
			.z_movement_keys = {
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RMETA, .keys = {KEY_TAB,0,0} }),
			KBRD_PRS_RLS({.modifier_keys = KEY_MOD_RMETA, .keys = {KEY_TAB,0,0} })}, }),
	.short_release = CFG(NO_REPORT,{}),
